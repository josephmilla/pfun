...
function doInsert()
{
	...
	if (strKey=="") {$('#noKey').modal(); return;}
	if (isNaN(strKey)) {$('#mustbeNumeric').modal(); return;}
	myBPT.bubbooID++;
...
function doFind()
{
	if (root === null) {$('#emptyTree').modal(); return;}
	var strFind = document.getElementById('findKey').value;
	if (strFind=="") {$('#noKey').modal(); return;}
	if (isNaN(strFind)) {$('#mustbeNumeric').modal(); return;}
	r = myBPT.find(root, strFind);
	if (r === null)
	{
		bootbox.alert("Record not found under key " + strFind);
	}
		
	else 
		bootbox.alert("Record at key " + strFind + " value " + r.value);
}
...
function doFindRange()
{
	...
	if (strMin=="") {$('#noMin').modal(); return;}
	if (isNaN(strMin)) {$('#min_mustbeNumeric').modal(); return;}
	
	var strMax = document.getElementById('maxRange').value;
	if (strMax=="") {$('#noMax').modal(); return;}
	if (isNaN(strMax)) {$('#max_mustbeNumeric').modal(); return;}	
	
	if (strMin > strMax) {$('#range_notValid').modal(); return;}
	
	var str_r = myBPT.findRange(root, strMin, strMax);
	if (str_r === null)
		bootbox.alert("No records found in the range [" + strMin + ", " + strMax + "]");
	else 
		bootbox.alert("Records found in the range [" + strMin + ", " + strMax + "] are " + str_r);
}
